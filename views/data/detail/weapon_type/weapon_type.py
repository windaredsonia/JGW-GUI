...
from views.data.detail.weapon_type.delete_weapon_type import delete_weapon_type

class WeaponType(tk.Frame):
    def __init__(self, controller):
        ...
        title_label = tk.Label(self, text="JENIS SENJATA", fg="white", bg="#3a4766", font=title_font)
        ...
        add_button = tk.Button(self, text="Tambah Jenis Senjata", bg="#ff9900", fg="white", 
                               font=FontConfig.get_button_font(size=15), command=self.open_add_weapon_window)
    ...
    def load_weapon_list(self):
        ...
        for index, weapon in enumerate(weapon_slice):
            ...
            delete_button = tk.Button(weapon_frame, text="Hapus", bg="#ff9900", fg="white", 
                                      font=FontConfig.get_button_font(size=15), command=lambda w=weapon: self.delete_weapon(w))
    ...
    def open_add_weapon_window(self):
        add_window = tk.Toplevel(self)
        add_window.title("Tambah Jenis Senjata")
        add_window.geometry("300x200")
        add_window.config(bg="#3a4766")

        label = tk.Label(add_window, text="Tambahkan Jenis yang Kamu Inginkan:", fg="white", bg="#3a4766", font=FontConfig.get_custom_font(size=10))
        label.pack(pady=10)
    ...
    def save_weapon(self, window):
        ...
        if new_weapon:
            ...
            tk.messagebox.showwarning("Warning", "Jenis senjata tidak boleh kosong!")

    def delete_weapon(self, weapon):
        if delete_weapon_type(weapon, self.weapon_types):
            self.load_weapon_list()
            self.update_nav_buttons()
